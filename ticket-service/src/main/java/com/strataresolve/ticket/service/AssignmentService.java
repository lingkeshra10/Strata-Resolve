...
import com.strataresolve.ticket.client.model.Role;
...
public class AssignmentService {
    ...
    private static final Set<String> TECHNICIAN_ROLES = Set.of(Role.TECHNICIAN);
    private static final Set<String> VENDOR_ROLES = Set.of(Role.VENDOR_ADMIN, Role.VENDOR_TECHNICIAN);
    ...
    public Assignment createAssignment(CreateAssignmentRequest request, UUID actingUserId) {
        ...
        List<Membership> activeMemberships = identityServiceClient
                .findActiveMemberships(request.assignedTo(), propertyId);
    ...
    private void validateAssigneeRole(List<Membership> memberships, AssignmentType type, UUID assigneeId) {
        Set<String> requiredRoles = type == AssignmentType.TECHNICIAN ? TECHNICIAN_ROLES : VENDOR_ROLES;

        boolean hasMatchingRole = memberships.stream()
                .anyMatch(m -> requiredRoles.contains(m.role()));
