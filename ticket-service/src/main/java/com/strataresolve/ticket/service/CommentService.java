...
public class CommentService {
     ...
     */
    private static final Set<String> INTERNAL_NOTE_ROLES = Set.of(
     ...
     */
    private static final Set<String> RESIDENT_ROLES = Set.of(
            Role.RESIDENT_OWNER,
    ...
    private void validateCanCreateInternalNote(UUID userId, UUID propertyId) {
        List<Membership> memberships = identityServiceClient.findActiveMemberships(userId, propertyId);
        boolean hasInternalNoteRole = memberships.stream()
                .anyMatch(membership -> INTERNAL_NOTE_ROLES.contains(membership.role()));
    ...
    private boolean isResident(UUID userId, UUID propertyId) {
        ...
        return memberships.stream().allMatch(membership -> RESIDENT_ROLES.contains(membership.role()));
    }
}
