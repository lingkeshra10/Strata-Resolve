...
import com.strataresolve.common.exception.InvalidTransitionException;
import com.strataresolve.common.exception.RateLimitExceededException;
import com.strataresolve.common.exception.ResourceNotFoundException;
import com.strataresolve.ticket.client.IdentityServiceClient;
import com.strataresolve.ticket.client.PropertyServiceClient;
import com.strataresolve.ticket.client.SlaServiceClient;
...
import com.strataresolve.ticket.dto.*;
...
public class TicketService {
    ...
    public Ticket submitTicket(CreateTicketRequest request, UUID propertyId, UUID submittedBy) {
        ...
        Property property = propertyServiceClient.findById(propertyId)
                .orElseThrow(() -> new ResourceNotFoundException("Property", propertyId));

        if (!property.active()) {
            throw new BusinessRuleViolationException(
                    "Cannot submit tickets to an inactive property. Property '" + property.name() + "' is currently inactive.");
        }
        ...
        List<Membership> activeMemberships = identityServiceClient.findActiveMemberships(submittedBy, propertyId);
        ...
        UUID unitId = residentMembership.unitId();
        ...
        SlaTargets slaTargets = slaServiceClient.calculateTargets(propertyId, property.timezone(),
    ...
    public Ticket changeCategory(UUID ticketId, Category newCategory, UUID actingUserId) {
        ...
        Property property = propertyServiceClient.findById(ticket.getPropertyId())
                .orElseThrow(() -> new ResourceNotFoundException("Property", ticket.getPropertyId()));

        SlaTargets slaTargets = slaServiceClient.calculateTargets(ticket.getPropertyId(),
                property.timezone(), newCategory.name(), ticket.getPriority().name());
    ...
    public Ticket changePriority(UUID ticketId, Priority newPriority, UUID actingUserId) {
        ...
        SlaTargets slaTargets = slaServiceClient.calculateTargets(ticket.getPropertyId(),
                property.timezone(), ticket.getCategory().name(), newPriority.name());
    ...
    public List<Ticket> findResidentTickets(UUID propertyId, UUID residentId) {
        List<Membership> activeMemberships = identityServiceClient
                .findActiveMemberships(residentId, propertyId);
        List<UUID> unitIds = activeMemberships.stream()
                .map(Membership::unitId)
